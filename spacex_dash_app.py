...
def get_scatter_chart(entered_site, entered_payload_kg):
    filtered_df = spacex_df
    title = "Correlation between Payload and success"
    if entered_site == 'ALL':
        title = title + " for all sites"
    else:
        filtered_df = filtered_df[filtered_df['Launch Site'] == entered_site]
        title = title + f" for {entered_site}"
    
    print(entered_payload_kg)
    filtered_df = filtered_df[(filtered_df['Payload Mass (kg)'] >= entered_payload_kg[0]) & (filtered_df['Payload Mass (kg)'] <= entered_payload_kg[1])]
